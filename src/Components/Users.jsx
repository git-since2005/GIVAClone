...
function Users() {
  ...
  const [form, setForm] = useState({ title: "", desc: "", price: 0, "quantity":0, imageLink:"" });
  ...
  async function fetchCustomerOrders() {
    ...
    let response = await fetch(`${import.meta.env.VITE_API_URL}/customerOrders`, {
      method:"POST",
      headers:{
        "Content-type":"application/json"
      },
    ...
    <div className=" flex flex-col ">
      ...
      <div className=" flex flex-wrap w-[70%] m-auto p-10 gap-5 ">
        {Object.keys(orders).map((e) => {
          ...
          return <VendorOrder key={orders[e].order_id} id={orders[e].order_id} title={orders[e].title} user_id={orders[e].user_id} use="admin" />;
        })}
      </div>
      <h1 className=" text-3xl font-thin m-auto ml-20 mt-7 mb-7 ">
        Your owned products,
      ...
      <div className=" flex flex-wrap w-[70%] m-auto p-10 gap-5 ">
        {Object.keys(products).map((e) => {
          return <OwnedProduct key={products[e].id} id={products[e].productId} title={products[e].title} desc={products[e].desc} price={products[e].price} imgLink = {products[e].imageLink ? true : false} img = {products[e].image || products[e].imageLink} use="admin" />;
        })}
      </div>
      <div className=" border-3 rounded-xl flex flex-col m-auto p-10 gap-5 ">
        ...
        <input
          ...
          placeholder="Title"
        />
        <input type="number" name="quantity" className=" border rounded p-2 outline-none " placeholder="Quantity" onChange={(e)=>setForm({...form, [e.target.name]:e.target.value})} />
        <input
          type="text"
        ...
        }} name="imageLink" className=" border rounded p-2 outline-none " placeholder="or URL for Image" />
        <button
          onClick={upload}
      ...
      <button onClick={()=>{localStorage.removeItem("user");window.location.pathname="login"}} className=" border-0 rounded-lg m-auto bg-red-500 cursor-pointer ml-10 mb-2 p-2 ">Sign Out</button>
    </div>
  );
}
